...
void w_ecs_world_init(struct w_ecs_world *world, struct w_string_table *string_table, struct w_arena *arena)
{
	...
	w_array_init_t(world->module_resources, W_MODULE_RESOURCES_INITIAL_CAPACITY);
...
void w_ecs_world_free(struct w_ecs_world *world)
{
	...
	free_null(world->module_resources);
...
{
	// get phase to find its timestep
	struct w_scheduler_phase *phase = w_scheduler_get_phase(&world->scheduler, phase_id);
	if (!phase) return;

	struct w_scheduler_time_step *ts = w_scheduler_get_time_step(&world->scheduler, phase->time_step_id);
	double delta_time = ts ? ts->time_step.delta_time_fixed : 0.0;

	// iterate all systems, run those matching this phase
	for (size_t i = 0; i < world->systems.systems_length; ++i)
	{
		struct w_system *sys = &world->systems.systems[i];
		if (!sys->enabled) continue;
		if (sys->phase_id != phase_id) continue;

		sys->update(world, delta_time);
	}
}
...
enum W_WORLD_UPDATE_RESULT w_ecs_update(struct w_ecs_world *world)
{
	...
	struct w_hook_registry *hooks = world->hooks;
	...
	for (size_t i = 0; i < count; ++i)
	{
		...
		switch (action->action) {
			...
			case W_SCHEDULER_ACTIONS_SCHEDULE_BEGIN:
				w_hook_registry_run_hooks(&hooks[W_WORLD_HOOK_TYPE_UPDATE], W_WORLD_HOOK_UPDATE_BEGIN, world, action);
				break;
			case W_SCHEDULER_ACTIONS_SCHEDULE_END:
				w_hook_registry_run_hooks(&hooks[W_WORLD_HOOK_TYPE_UPDATE], W_WORLD_HOOK_UPDATE_END, world, action);
				break;
			case W_SCHEDULER_ACTIONS_TIMESTEP_BEGIN:
			{
				...
				w_hook_registry_run_hooks(&hooks[W_WORLD_HOOK_TYPE_UPDATE], W_WORLD_HOOK_UPDATE_TIMESTEP_BEGIN, world, action);
				break;
			}
			case W_SCHEDULER_ACTIONS_TIMESTEP_END:
				w_hook_registry_run_hooks(&hooks[W_WORLD_HOOK_TYPE_UPDATE], W_WORLD_HOOK_UPDATE_TIMESTEP_END, world, action);
			...
			case W_SCHEDULER_ACTIONS_PHASE_BEGIN:
				w_hook_registry_run_hooks(&hooks[W_WORLD_HOOK_TYPE_UPDATE], W_WORLD_HOOK_UPDATE_PHASE_BEGIN, world, action);
				break;
			case W_SCHEDULER_ACTIONS_PHASE_END:
				w_hook_registry_run_hooks(&hooks[W_WORLD_HOOK_TYPE_UPDATE], W_WORLD_HOOK_UPDATE_PHASE_END, world, action);
				break;
...
{
	struct w_hook_entry *entry = w_hook_registry_get_hook_entry(&world->hooks[W_WORLD_HOOK_TYPE_COMPONENT_SET], type_id, hook_id);
	if (entry) entry->enabled = false;
}

size_t w_ecs_register_component_remove_hook(struct w_ecs_world *world, uint type_id, w_hook_fn hook_fn)
{
	return w_hook_registry_register_hook(&world->hooks[W_WORLD_HOOK_TYPE_COMPONENT_REMOVE], type_id, hook_fn);
}

void w_ecs_unregister_component_remove_hook(struct w_ecs_world *world, uint type_id, size_t hook_id)
{
	struct w_hook_entry *entry = w_hook_registry_get_hook_entry(&world->hooks[W_WORLD_HOOK_TYPE_COMPONENT_REMOVE], type_id, hook_id);
	if (entry) entry->enabled = false;
}

size_t w_ecs_register_component_pre_set_hook(struct w_ecs_world *world, w_hook_fn hook_fn)
{
	return w_hook_registry_register_hook(&world->hooks[W_WORLD_HOOK_TYPE_COMPONENT_PRE_SET], 0, hook_fn);
}

void w_ecs_unregister_component_pre_set_hook(struct w_ecs_world *world, size_t hook_id)
{
	struct w_hook_entry *entry = w_hook_registry_get_hook_entry(&world->hooks[W_WORLD_HOOK_TYPE_COMPONENT_PRE_SET], 0, hook_id);
	if (entry) entry->enabled = false;
}

size_t w_ecs_register_component_pre_remove_hook(struct w_ecs_world *world, w_hook_fn hook_fn)
{
	return w_hook_registry_register_hook(&world->hooks[W_WORLD_HOOK_TYPE_COMPONENT_PRE_REMOVE], 0, hook_fn);
}
