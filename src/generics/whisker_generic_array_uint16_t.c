...
// create an instance of a managed array with type uint16_t
E_WHISKER_ARR whisker_arr_create_uint16_t(whisker_arr_uint16_t **arr, size_t length)
{
	whisker_arr_uint16_t *a = whisker_mem_xcalloc_t(1, *a);
	a->length = length;
...
// init an instance of a managed array with type uint16_t
E_WHISKER_ARR whisker_arr_init_uint16_t(whisker_arr_uint16_t *arr)
{
	if (arr->length > 0 && arr->arr == NULL) {  \
		arr->arr = whisker_mem_xcalloc(1, sizeof(uint16_t) * arr->length);
	}
	return E_WHISKER_ARR_OK;
}

// deallocate an instance of a managed array with type uint16_t
...
// resize a managed array of type uint16_t
E_WHISKER_ARR whisker_arr_resize_uint16_t(whisker_arr_uint16_t *arr, size_t length, bool soft_resize)
{
	if (length == 0 && arr->alloc_size > 0 && !soft_resize) {
		...
		arr->arr = whisker_mem_xrealloc(arr->arr, length * sizeof(uint16_t));
...
// push a uint16_t value into the array, resizing and re-allocating if required
...
// pop the last uint16_t value from the array, decreasing the managed length
...
// swap 2 uint16_t values with the given indexes using the swap buffer
...
// get the index of the uint16_t value, -1 if it doesn't exist
