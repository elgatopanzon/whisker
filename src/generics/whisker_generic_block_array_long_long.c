...
#include "whisker_generic_block_array_long_long.h"
//
E_WHISKER_BLOCK_ARR whisker_block_arr_create_long_long(whisker_block_arr_long_long **barr, size_t block_size)
{
	whisker_block_arr_long_long *ba;
	E_WHISKER_MEM err = whisker_mem_try_calloc(1, sizeof(whisker_block_arr_long_long), (void**)&ba);
...
void whisker_block_arr_free_long_long(whisker_block_arr_long_long *barr)
{
	for (size_t i = 0; i < barr->blocks->length; ++i)
	{
		...
		{
			whisker_arr_long_long *block = barr->blocks->arr[i];
			whisker_arr_free_long_long(block);
		}
	}
...
E_WHISKER_BLOCK_ARR whisker_block_arr_set_long_long(whisker_block_arr_long_long *barr, size_t index, long long value)
{
	...
	if (barr->blocks->length < index + 1 || ((whisker_arr_long_long *)barr->blocks->arr[block_id])->arr == NULL) {
		E_WHISKER_BLOCK_ARR err = whisker_block_arr_init_block_long_long(barr, block_id);
		if (err != E_WHISKER_BLOCK_ARR_OK) { return err; }
	}
	((whisker_arr_long_long *)barr->blocks->arr[block_id])->arr[block_offset] = value;
	return E_WHISKER_BLOCK_ARR_OK;
}
...
E_WHISKER_BLOCK_ARR whisker_block_arr_init_block_long_long(whisker_block_arr_long_long *barr, size_t block_id)
{
	...
	if (barr->blocks->arr[block_id] == NULL) {
		whisker_arr_long_long *block;
		E_WHISKER_ARR err = whisker_arr_create_long_long(&block, barr->block_size);
		if (err != E_WHISKER_ARR_OK) { return E_WHISKER_BLOCK_ARR_MEM; }
...
long long whisker_block_arr_get_long_long(whisker_block_arr_long_long *barr, size_t index)
{
	size_t block_id = whisker_block_arr_get_block_id(barr->block_size, index);
	size_t block_offset = whisker_block_arr_get_block_offset(barr->block_size, index);
	return ((whisker_arr_long_long *)barr->blocks->arr[block_id])->arr[block_offset];
}

bool whisker_block_arr_contains_long_long(whisker_block_arr_long_long *barr, size_t index)
{
	size_t block_id = whisker_block_arr_get_block_id(barr->block_size, index);
	return barr->blocks->length > block_id + 1 && ((whisker_arr_long_long *)barr->blocks->arr[block_id])->arr != NULL;
}
