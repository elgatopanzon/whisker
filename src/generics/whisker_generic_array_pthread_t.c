...
#include "whisker_generic_array_pthread_t.h"

// create an instance of a managed array with type pthread_t
E_WHISKER_ARR whisker_arr_create_pthread_t(whisker_arr_pthread_t **arr, size_t length)
{
	whisker_arr_pthread_t *a;
	E_WHISKER_MEM err = whisker_mem_try_calloc(1, sizeof(whisker_arr_pthread_t), (void**)&a);
	if (err != E_WHISKER_MEM_OK) { return E_WHISKER_ARR_MEM; }
	a->length = length;
	a->alloc_size = sizeof(pthread_t) * a->length;
	if (length > 0) { whisker_arr_init_pthread_t(a); }
	*arr = a;
	return E_WHISKER_ARR_OK;
}

// init an instance of a managed array with type pthread_t
E_WHISKER_ARR whisker_arr_init_pthread_t(whisker_arr_pthread_t *arr)
{
	if (arr->length > 0 && arr->arr == NULL) {  \
		E_WHISKER_MEM err = whisker_mem_try_calloc(1, sizeof(pthread_t) * arr->length, (void**)&arr->arr);
		if (err != E_WHISKER_MEM_OK) { return E_WHISKER_ARR_MEM; }
	}
	return E_WHISKER_ARR_OK;
}

// deallocate an instance of a managed array with type pthread_t
...
// resize a managed array of type pthread_t
E_WHISKER_ARR whisker_arr_resize_pthread_t(whisker_arr_pthread_t *arr, size_t length, bool soft_resize)
{
	...
	} else if ((arr->length > length && soft_resize) || arr->alloc_size >= (length * sizeof(pthread_t))) {
		arr->length = length;
		return E_WHISKER_ARR_OK;
	} else {
		E_WHISKER_MEM err = whisker_mem_try_realloc(arr->arr, length * sizeof(pthread_t), (void**)&arr->arr);
		if (err != E_WHISKER_MEM_OK) {
			return E_WHISKER_ARR_MEM;
		}
		memset(((unsigned char*)arr->arr) + arr->alloc_size, 0, (length * sizeof(pthread_t)) - arr->alloc_size);
		arr->alloc_size = length * sizeof(pthread_t);
		arr->length = length;
	}
	return E_WHISKER_ARR_OK;
}
...
{
	return whisker_arr_resize_pthread_t(arr, arr->length + 1, true);
}
...
// push a pthread_t value into the array, resizing and re-allocating if required
E_WHISKER_ARR whisker_arr_push_pthread_t(whisker_arr_pthread_t *arr, pthread_t value)
{
	E_WHISKER_ARR err = whisker_arr_increment_size_pthread_t(arr);
	if (err != E_WHISKER_ARR_OK) { return err; }
	arr->arr[arr->length - 1] = value;
	return E_WHISKER_ARR_OK;
}

// pop the last pthread_t value from the array, decreasing the managed length
...
{
	E_WHISKER_ARR err = whisker_arr_decrement_size_pthread_t(arr);
	if (err != E_WHISKER_ARR_OK) { return err; }
	pthread_t popped = arr->arr[arr->length];
	*out = popped;
	return E_WHISKER_ARR_OK;
}

// swap 2 pthread_t values with the given indexes using the swap buffer
E_WHISKER_ARR whisker_arr_swap_pthread_t(whisker_arr_pthread_t *arr, size_t index_a, size_t index_b)
{
	...
	memset(&arr->swap_buffer, 0x00, sizeof(pthread_t));
	return E_WHISKER_ARR_OK;
}
...
{
	arr->length = 0;
	if (compact) { whisker_arr_resize_pthread_t(arr, 0, false); }
}
...
{
	return whisker_arr_resize_pthread_t(arr, arr->length, false);
}
...
// get the index of the pthread_t value, -1 if it doesn't exist
