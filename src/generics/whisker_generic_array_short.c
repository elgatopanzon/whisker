...
// create an instance of a managed array with type short
E_WHISKER_ARR whisker_arr_create_short(whisker_arr_short **arr, size_t length)
{
	whisker_arr_short *a = whisker_mem_xcalloc_t(1, *a);
	a->length = length;
...
// init an instance of a managed array with type short
E_WHISKER_ARR whisker_arr_init_short(whisker_arr_short *arr)
{
	if (arr->length > 0 && arr->arr == NULL) {  \
		arr->arr = whisker_mem_xcalloc(1, sizeof(short) * arr->length);
	}
	return E_WHISKER_ARR_OK;
}

// deallocate an instance of a managed array with type short
...
// resize a managed array of type short
E_WHISKER_ARR whisker_arr_resize_short(whisker_arr_short *arr, size_t length, bool soft_resize)
{
	if (length == 0 && arr->alloc_size > 0 && !soft_resize) {
		...
		arr->arr = whisker_mem_xrealloc(arr->arr, length * sizeof(short));
...
// push a short value into the array, resizing and re-allocating if required
...
// pop the last short value from the array, decreasing the managed length
...
// swap 2 short values with the given indexes using the swap buffer
...
// get the index of the short value, -1 if it doesn't exist
