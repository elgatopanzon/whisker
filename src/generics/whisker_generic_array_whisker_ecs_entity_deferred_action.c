...
#include "whisker_generic_array_whisker_ecs_entity_deferred_action.h"

// create an instance of a managed array with type whisker_ecs_entity_deferred_action
...
{
	whisker_arr_whisker_ecs_entity_deferred_action *a;
	E_WHISKER_MEM err = whisker_mem_try_calloc(1, sizeof(whisker_arr_whisker_ecs_entity_deferred_action), (void**)&a);
	if (err != E_WHISKER_MEM_OK) { return E_WHISKER_ARR_MEM; }
	a->length = length;
	a->alloc_size = sizeof(whisker_ecs_entity_deferred_action) * a->length;
	if (length > 0) { whisker_arr_init_whisker_ecs_entity_deferred_action(a); }
	*arr = a;
	return E_WHISKER_ARR_OK;
}

// init an instance of a managed array with type whisker_ecs_entity_deferred_action
E_WHISKER_ARR whisker_arr_init_whisker_ecs_entity_deferred_action(whisker_arr_whisker_ecs_entity_deferred_action *arr)
{
	if (arr->length > 0 && arr->arr == NULL) {  \
		E_WHISKER_MEM err = whisker_mem_try_calloc(1, sizeof(whisker_ecs_entity_deferred_action) * arr->length, (void**)&arr->arr);
		if (err != E_WHISKER_MEM_OK) { return E_WHISKER_ARR_MEM; }
	}
	return E_WHISKER_ARR_OK;
}

// deallocate an instance of a managed array with type whisker_ecs_entity_deferred_action
...
// resize a managed array of type whisker_ecs_entity_deferred_action
E_WHISKER_ARR whisker_arr_resize_whisker_ecs_entity_deferred_action(whisker_arr_whisker_ecs_entity_deferred_action *arr, size_t length, bool soft_resize)
{
	...
	} else if ((arr->length > length && soft_resize) || arr->alloc_size >= (length * sizeof(whisker_ecs_entity_deferred_action))) {
		arr->length = length;
		return E_WHISKER_ARR_OK;
	} else {
		E_WHISKER_MEM err = whisker_mem_try_realloc(arr->arr, length * sizeof(whisker_ecs_entity_deferred_action), (void**)&arr->arr);
		if (err != E_WHISKER_MEM_OK) {
			return E_WHISKER_ARR_MEM;
		}
		memset(((unsigned char*)arr->arr) + arr->alloc_size, 0, (length * sizeof(whisker_ecs_entity_deferred_action)) - arr->alloc_size);
		arr->alloc_size = length * sizeof(whisker_ecs_entity_deferred_action);
		arr->length = length;
	}
	return E_WHISKER_ARR_OK;
}
...
{
	return whisker_arr_resize_whisker_ecs_entity_deferred_action(arr, arr->length + 1, true);
}
...
// push a whisker_ecs_entity_deferred_action value into the array, resizing and re-allocating if required
...
{
	E_WHISKER_ARR err = whisker_arr_increment_size_whisker_ecs_entity_deferred_action(arr);
	if (err != E_WHISKER_ARR_OK) { return err; }
	arr->arr[arr->length - 1] = value;
	return E_WHISKER_ARR_OK;
}

// pop the last whisker_ecs_entity_deferred_action value from the array, decreasing the managed length
...
{
	E_WHISKER_ARR err = whisker_arr_decrement_size_whisker_ecs_entity_deferred_action(arr);
	if (err != E_WHISKER_ARR_OK) { return err; }
	whisker_ecs_entity_deferred_action popped = arr->arr[arr->length];
	*out = popped;
	return E_WHISKER_ARR_OK;
}

// swap 2 whisker_ecs_entity_deferred_action values with the given indexes using the swap buffer
E_WHISKER_ARR whisker_arr_swap_whisker_ecs_entity_deferred_action(whisker_arr_whisker_ecs_entity_deferred_action *arr, size_t index_a, size_t index_b)
{
	...
	memset(&arr->swap_buffer, 0x00, sizeof(whisker_ecs_entity_deferred_action));
	return E_WHISKER_ARR_OK;
}
...
{
	arr->length = 0;
	if (compact) { whisker_arr_resize_whisker_ecs_entity_deferred_action(arr, 0, false); }
}
...
{
	return whisker_arr_resize_whisker_ecs_entity_deferred_action(arr, arr->length, false);
}
...
// get the index of the whisker_ecs_entity_deferred_action value, -1 if it doesn't exist
