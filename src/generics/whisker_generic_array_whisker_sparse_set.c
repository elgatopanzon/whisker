...
#include "whisker_generic_array_whisker_sparse_set.h"
...
// init an instance of a managed array with type whisker_sparse_set
...
// deallocate an instance of a managed array with type whisker_sparse_set
...
// resize a managed array of type whisker_sparse_set
E_WHISKER_ARR whisker_arr_resize_whisker_sparse_set(whisker_arr_whisker_sparse_set *arr, size_t length, bool soft_resize)
{
	...
	} else if ((arr->length > length && soft_resize) || arr->alloc_size >= (length * sizeof(whisker_sparse_set))) {
		arr->length = length;
		return E_WHISKER_ARR_OK;
	} else {
...
{
	return whisker_arr_resize_whisker_sparse_set(arr, arr->length + 1, true);
}
...
// push a whisker_sparse_set value into the array, resizing and re-allocating if required
E_WHISKER_ARR whisker_arr_push_whisker_sparse_set(whisker_arr_whisker_sparse_set *arr, whisker_sparse_set value)
{
	E_WHISKER_ARR err = whisker_arr_increment_size_whisker_sparse_set(arr);
	if (err != E_WHISKER_ARR_OK) { return err; }
	arr->arr[arr->length - 1] = value;
	return E_WHISKER_ARR_OK;
}

// pop the last whisker_sparse_set value from the array, decreasing the managed length
...
{
	E_WHISKER_ARR err = whisker_arr_decrement_size_whisker_sparse_set(arr);
	if (err != E_WHISKER_ARR_OK) { return err; }
	whisker_sparse_set popped = arr->arr[arr->length];
	*out = popped;
	return E_WHISKER_ARR_OK;
}

// swap 2 whisker_sparse_set values with the given indexes using the swap buffer
E_WHISKER_ARR whisker_arr_swap_whisker_sparse_set(whisker_arr_whisker_sparse_set *arr, size_t index_a, size_t index_b)
{
	...
	memset(&arr->swap_buffer, 0x00, sizeof(whisker_sparse_set));
	return E_WHISKER_ARR_OK;
}
...
{
	arr->length = 0;
	if (compact) { whisker_arr_resize_whisker_sparse_set(arr, 0, false); }
}
...
{
	return whisker_arr_resize_whisker_sparse_set(arr, arr->length, false);
}
...
// get the index of the whisker_sparse_set value, -1 if it doesn't exist
...
{
	return whisker_arr_resize_whisker_sparse_set_ptr(arr, arr->length + 1, true);
}
...
{
	E_WHISKER_ARR err = whisker_arr_increment_size_whisker_sparse_set_ptr(arr);
	if (err != E_WHISKER_ARR_OK) { return err; }
	arr->arr[arr->length - 1] = value;
	return E_WHISKER_ARR_OK;
}
...
{
	E_WHISKER_ARR err = whisker_arr_decrement_size_whisker_sparse_set_ptr(arr);
	if (err != E_WHISKER_ARR_OK) { return err; }
	whisker_sparse_set *popped = arr->arr[arr->length];
	*out = popped;
	return E_WHISKER_ARR_OK;
...
{
	arr->length = 0;
	if (compact) { whisker_arr_resize_whisker_sparse_set_ptr(arr, 0, false); }
}

// compact and resize the allocation to fit the managed size
E_WHISKER_ARR whisker_arr_compact_whisker_sparse_set_ptr(whisker_arr_whisker_sparse_set_ptr *arr)
{
	return whisker_arr_resize_whisker_sparse_set_ptr(arr, arr->length, false);
}
