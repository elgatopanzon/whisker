...
#include "whisker_generic_array_unsigned_long.h"

// create an instance of a managed array with type unsigned long
E_WHISKER_ARR whisker_arr_create_unsigned_long(whisker_arr_unsigned_long **arr, size_t length)
{
	whisker_arr_unsigned_long *a;
	E_WHISKER_MEM err = whisker_mem_try_calloc(1, sizeof(whisker_arr_unsigned_long), (void**)&a);
	if (err != E_WHISKER_MEM_OK) { return E_WHISKER_ARR_MEM; }
	a->length = length;
	a->alloc_size = sizeof(unsigned long) * a->length;
	if (length > 0) { whisker_arr_init_unsigned_long(a); }
	*arr = a;
	return E_WHISKER_ARR_OK;
}

// init an instance of a managed array with type unsigned long
E_WHISKER_ARR whisker_arr_init_unsigned_long(whisker_arr_unsigned_long *arr)
{
	if (arr->length > 0 && arr->arr == NULL) {  \
		E_WHISKER_MEM err = whisker_mem_try_calloc(1, sizeof(unsigned long) * arr->length, (void**)&arr->arr);
		if (err != E_WHISKER_MEM_OK) { return E_WHISKER_ARR_MEM; }
	}
	return E_WHISKER_ARR_OK;
}

// deallocate an instance of a managed array with type unsigned long
...
// resize a managed array of type unsigned long
E_WHISKER_ARR whisker_arr_resize_unsigned_long(whisker_arr_unsigned_long *arr, size_t length, bool soft_resize)
{
	...
	} else if ((arr->length > length && soft_resize) || arr->alloc_size >= (length * sizeof(unsigned long))) {
		arr->length = length;
		return E_WHISKER_ARR_OK;
	} else {
		E_WHISKER_MEM err = whisker_mem_try_realloc(arr->arr, length * sizeof(unsigned long), (void**)&arr->arr);
		if (err != E_WHISKER_MEM_OK) {
			return E_WHISKER_ARR_MEM;
		}
		memset(((unsigned char*)arr->arr) + arr->alloc_size, 0, (length * sizeof(unsigned long)) - arr->alloc_size);
		arr->alloc_size = length * sizeof(unsigned long);
		arr->length = length;
	}
	return E_WHISKER_ARR_OK;
}
...
{
	return whisker_arr_resize_unsigned_long(arr, arr->length + 1, true);
}
...
// push a unsigned long value into the array, resizing and re-allocating if required
E_WHISKER_ARR whisker_arr_push_unsigned_long(whisker_arr_unsigned_long *arr, unsigned long value)
{
	E_WHISKER_ARR err = whisker_arr_increment_size_unsigned_long(arr);
	if (err != E_WHISKER_ARR_OK) { return err; }
	arr->arr[arr->length - 1] = value;
	return E_WHISKER_ARR_OK;
}

// pop the last unsigned long value from the array, decreasing the managed length
...
{
	E_WHISKER_ARR err = whisker_arr_decrement_size_unsigned_long(arr);
	if (err != E_WHISKER_ARR_OK) { return err; }
	unsigned long popped = arr->arr[arr->length];
	*out = popped;
	return E_WHISKER_ARR_OK;
}

// swap 2 unsigned long values with the given indexes using the swap buffer
E_WHISKER_ARR whisker_arr_swap_unsigned_long(whisker_arr_unsigned_long *arr, size_t index_a, size_t index_b)
{
	...
	memset(&arr->swap_buffer, 0x00, sizeof(unsigned long));
	return E_WHISKER_ARR_OK;
}
...
{
	arr->length = 0;
	if (compact) { whisker_arr_resize_unsigned_long(arr, 0, false); }
}
...
{
	return whisker_arr_resize_unsigned_long(arr, arr->length, false);
}
...
// get the index of the unsigned long value, -1 if it doesn't exist
