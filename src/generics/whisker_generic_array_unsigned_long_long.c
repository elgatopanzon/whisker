...
// create an instance of a managed array with type unsigned long long
E_WHISKER_ARR whisker_arr_create_unsigned_long_long(whisker_arr_unsigned_long_long **arr, size_t length)
{
	whisker_arr_unsigned_long_long *a = whisker_mem_xcalloc_t(1, *a);
	a->length = length;
...
// init an instance of a managed array with type unsigned long long
E_WHISKER_ARR whisker_arr_init_unsigned_long_long(whisker_arr_unsigned_long_long *arr)
{
	if (arr->length > 0 && arr->arr == NULL) {  \
		arr->arr = whisker_mem_xcalloc(1, sizeof(unsigned long long) * arr->length);
	}
	return E_WHISKER_ARR_OK;
}

// deallocate an instance of a managed array with type unsigned long long
...
// resize a managed array of type unsigned long long
E_WHISKER_ARR whisker_arr_resize_unsigned_long_long(whisker_arr_unsigned_long_long *arr, size_t length, bool soft_resize)
{
	if (length == 0 && arr->alloc_size > 0 && !soft_resize) {
		...
		arr->arr = whisker_mem_xrealloc(arr->arr, length * sizeof(unsigned long long));
...
// push a unsigned long long value into the array, resizing and re-allocating if required
...
// pop the last unsigned long long value from the array, decreasing the managed length
...
// swap 2 unsigned long long values with the given indexes using the swap buffer
...
// get the index of the unsigned long long value, -1 if it doesn't exist
