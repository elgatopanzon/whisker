...
{
	whisker_arr_char *a;
	E_WHISKER_MEM err = whisker_mem_try_calloc(1, sizeof(whisker_arr_char), (void**)&a);
...
E_WHISKER_ARR whisker_arr_init_char(whisker_arr_char *arr)
{
	if (arr->length > 0 && arr->arr == NULL) {  \
		E_WHISKER_MEM err = whisker_mem_try_calloc(1, sizeof(char) * arr->length, (void**)&arr->arr);
		if (err != E_WHISKER_MEM_OK) { return E_WHISKER_ARR_MEM; }
	}
	return E_WHISKER_ARR_OK;
}
...
E_WHISKER_ARR whisker_arr_resize_char(whisker_arr_char *arr, size_t length, bool soft_resize)
{
	if (length == 0 && arr->alloc_size > 0 && !soft_resize) {
		...
		E_WHISKER_MEM err = whisker_mem_try_realloc(arr->arr, length * sizeof(char), (void**)&arr->arr);
