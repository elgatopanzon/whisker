...
// create an instance of a managed array with type whisker_ecs_entity
E_WHISKER_ARR whisker_arr_create_whisker_ecs_entity(whisker_arr_whisker_ecs_entity **arr, size_t length)
{
	whisker_arr_whisker_ecs_entity *a = whisker_mem_xcalloc_t(1, *a);
	a->length = length;
...
// init an instance of a managed array with type whisker_ecs_entity
E_WHISKER_ARR whisker_arr_init_whisker_ecs_entity(whisker_arr_whisker_ecs_entity *arr)
{
	if (arr->length > 0 && arr->arr == NULL) {  \
		arr->arr = whisker_mem_xcalloc(1, sizeof(whisker_ecs_entity) * arr->length);
	}
	return E_WHISKER_ARR_OK;
}

// deallocate an instance of a managed array with type whisker_ecs_entity
...
// resize a managed array of type whisker_ecs_entity
E_WHISKER_ARR whisker_arr_resize_whisker_ecs_entity(whisker_arr_whisker_ecs_entity *arr, size_t length, bool soft_resize)
{
	if (length == 0 && arr->alloc_size > 0 && !soft_resize) {
		...
		arr->arr = whisker_mem_xrealloc(arr->arr, length * sizeof(whisker_ecs_entity));
...
// push a whisker_ecs_entity value into the array, resizing and re-allocating if required
...
// pop the last whisker_ecs_entity value from the array, decreasing the managed length
...
// swap 2 whisker_ecs_entity values with the given indexes using the swap buffer
...
// get the index of the whisker_ecs_entity value, -1 if it doesn't exist
