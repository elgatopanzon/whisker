...
#include "whisker_generic_block_array_uint16_t.h"
//
E_WHISKER_BLOCK_ARR whisker_block_arr_create_uint16_t(whisker_block_arr_uint16_t **barr, size_t block_size)
{
	whisker_block_arr_uint16_t *ba;
	E_WHISKER_MEM err = whisker_mem_try_calloc(1, sizeof(whisker_block_arr_uint16_t), (void**)&ba);
...
void whisker_block_arr_free_uint16_t(whisker_block_arr_uint16_t *barr)
{
	for (size_t i = 0; i < barr->blocks->length; ++i)
	{
		...
		{
			whisker_arr_uint16_t *block = barr->blocks->arr[i];
			whisker_arr_free_uint16_t(block);
		}
	}
...
E_WHISKER_BLOCK_ARR whisker_block_arr_set_uint16_t(whisker_block_arr_uint16_t *barr, size_t index, uint16_t value)
{
	...
	if (barr->blocks->length < index + 1 || ((whisker_arr_uint16_t *)barr->blocks->arr[block_id])->arr == NULL) {
		E_WHISKER_BLOCK_ARR err = whisker_block_arr_init_block_uint16_t(barr, block_id);
		if (err != E_WHISKER_BLOCK_ARR_OK) { return err; }
	}
	((whisker_arr_uint16_t *)barr->blocks->arr[block_id])->arr[block_offset] = value;
	return E_WHISKER_BLOCK_ARR_OK;
}
...
E_WHISKER_BLOCK_ARR whisker_block_arr_init_block_uint16_t(whisker_block_arr_uint16_t *barr, size_t block_id)
{
	...
	if (barr->blocks->arr[block_id] == NULL) {
		whisker_arr_uint16_t *block;
		E_WHISKER_ARR err = whisker_arr_create_uint16_t(&block, barr->block_size);
		if (err != E_WHISKER_ARR_OK) { return E_WHISKER_BLOCK_ARR_MEM; }
...
uint16_t whisker_block_arr_get_uint16_t(whisker_block_arr_uint16_t *barr, size_t index)
{
	size_t block_id = whisker_block_arr_get_block_id(barr->block_size, index);
	size_t block_offset = whisker_block_arr_get_block_offset(barr->block_size, index);
	return ((whisker_arr_uint16_t *)barr->blocks->arr[block_id])->arr[block_offset];
}

bool whisker_block_arr_contains_uint16_t(whisker_block_arr_uint16_t *barr, size_t index)
{
	size_t block_id = whisker_block_arr_get_block_id(barr->block_size, index);
	return barr->blocks->length > block_id + 1 && ((whisker_arr_uint16_t *)barr->blocks->arr[block_id])->arr != NULL;
}
