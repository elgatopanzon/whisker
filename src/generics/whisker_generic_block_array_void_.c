...
#include "whisker_generic_block_array_void_.h"
//
E_WHISKER_BLOCK_ARR whisker_block_arr_create_void_(whisker_block_arr_void_ **barr, size_t block_size)
{
	whisker_block_arr_void_ *ba;
	E_WHISKER_MEM err = whisker_mem_try_calloc(1, sizeof(whisker_block_arr_void_), (void**)&ba);
...
void whisker_block_arr_free_void_(whisker_block_arr_void_ *barr)
{
	for (size_t i = 0; i < barr->blocks->length; ++i)
	{
		...
		{
			whisker_arr_void_ *block = barr->blocks->arr[i];
			whisker_arr_free_void_(block);
		}
	}
...
E_WHISKER_BLOCK_ARR whisker_block_arr_set_void_(whisker_block_arr_void_ *barr, size_t index, void* value)
{
	...
	if (barr->blocks->length < index + 1 || ((whisker_arr_void_ *)barr->blocks->arr[block_id])->arr == NULL) {
		E_WHISKER_BLOCK_ARR err = whisker_block_arr_init_block_void_(barr, block_id);
		if (err != E_WHISKER_BLOCK_ARR_OK) { return err; }
	}
	((whisker_arr_void_ *)barr->blocks->arr[block_id])->arr[block_offset] = value;
	return E_WHISKER_BLOCK_ARR_OK;
}
...
E_WHISKER_BLOCK_ARR whisker_block_arr_init_block_void_(whisker_block_arr_void_ *barr, size_t block_id)
{
	...
	if (barr->blocks->arr[block_id] == NULL) {
		whisker_arr_void_ *block;
		E_WHISKER_ARR err = whisker_arr_create_void_(&block, barr->block_size);
		if (err != E_WHISKER_ARR_OK) { return E_WHISKER_BLOCK_ARR_MEM; }
...
void* whisker_block_arr_get_void_(whisker_block_arr_void_ *barr, size_t index)
{
	size_t block_id = whisker_block_arr_get_block_id(barr->block_size, index);
	size_t block_offset = whisker_block_arr_get_block_offset(barr->block_size, index);
	return ((whisker_arr_void_ *)barr->blocks->arr[block_id])->arr[block_offset];
}

bool whisker_block_arr_contains_void_(whisker_block_arr_void_ *barr, size_t index)
{
	size_t block_id = whisker_block_arr_get_block_id(barr->block_size, index);
	return barr->blocks->length > block_id + 1 && ((whisker_arr_void_ *)barr->blocks->arr[block_id])->arr != NULL;
}
