...
void whisker_ecs_module_component_change_events_system(whisker_ecs_system_context *context)
{
	...
	if (context->components->deferred_actions_length > 0) 
	{
		for (int i = 0; i < context->components->deferred_actions_length; ++i)
		{
			struct whisker_ecs_component_deferred_action *action = &context->components->deferred_actions[i];

			// holds the name of the event to create
			char* event_name; 
			char* event_name_target; 
			char* component_name = whisker_ecs_e(context->entities, action->component_id)->name;
			...
			switch (action->action) {
				case WHISKER_ECS_COMPONENT_DEFERRED_ACTION_SET:
					...
					if (!whisker_ecs_has_component(context->components, action->component_id, action->entity_id))
					{
					...
					else
					{
						...
						// skip this action
						if (memcmp(context->components->deferred_actions_data + action->data_offset, whisker_ss_get(context->components->components[action->component_id.index], action->entity_id.index), action->data_size) == 0)
						{
							continue;
						}
			...
			whisker_ecs_entity_id event_entity = whisker_ecs_e_create_named(context->entities, event_name);
			whisker_ecs_entity_id event_entity_target = whisker_ecs_e_create_named(context->entities, event_name_target);


			// first fire the targetted event as a new event
			whisker_ecs_entity_id event_target_entity = action->entity_id;
			whisker_ecs_entity_id ev = whisker_ecs_module_event_create_with_data(context->entities, context->components, event_entity_target, whisker_ecs_entity_id, &event_target_entity);
			whisker_ecs_module_event_fire(context->entities, context->components, ev);
			...
			// destruction
			whisker_ecs_module_event_fire_on_f(context->entities, context->components, event_entity, event_target_entity);
